...
const emptyForm = (): Omit<Athlete, 'id'> => ({
  name: '', age: null, position: '', email: '',
  phone: '', parent_name: '', parent_phone: '', injury_status: 'Healthy',
})

const injuryColors: Record<string, string> = {
  Healthy: 'bg-green-900/40 text-green-400 border border-green-700',
  Injured: 'bg-red-900/40 text-red-400 border border-red-700',
  'Modified Training': 'bg-yellow-900/40 text-yellow-400 border border-yellow-700',
}
...
export default function MyTeamPage() {
  ...
  function startEdit(a: Athlete) {
    setForm({ name: a.name, age: a.age, position: a.position, email: a.email,
      phone: a.phone, parent_name: a.parent_name, parent_phone: a.parent_phone, injury_status: a.injury_status })
    setEditId(a.id); setShowForm(true); setExpanded(null)
  }
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Starlings 2026 · {athletes.length} athlete{athletes.length !== 1 ? 's' : ''}</p>
        </div>
        <button onClick={() => { setShowForm(true); setForm(emptyForm()); setEditId(null) }}
          className="btn-red px-4 py-2 text-sm">+ Add Athlete</button>
      </div>
      ...
      {showForm && (
        <div className="card p-6 mb-6">
          <h3 className="text-white font-bold mb-4 text-lg">{editId ? 'Edit Athlete' : 'Add New Athlete'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {([
              ['name', 'Full Name *', 'text', 'Athlete name'],
              ['age', 'Age', 'number', '16'],
              ['position', 'Position', 'text', 'e.g. Flyer, Base'],
              ['email', 'Athlete Email', 'email', 'athlete@example.com'],
              ['phone', 'Athlete Phone', 'text', '+27 82 000 0000'],
              ['parent_name', 'Parent / Guardian Name', 'text', 'Parent name'],
              ['parent_phone', 'Parent Phone', 'text', '+27 82 000 0000'],
            ] as [keyof Omit<Athlete,'id'>, string, string, string][]).map(([key, label, type, placeholder]) => (
              <div key={key}>
                <label className="block text-xs text-gray-400 mb-1">{label}</label>
                <input type={type} placeholder={placeholder} value={(form as any)[key] ?? ''}
                  onChange={e => setForm(prev => ({ ...prev, [key]: type === 'number' ? (parseInt(e.target.value) || null) : e.target.value }))}
                  className="input" />
              </div>
            ))}
            <div>
          ...
          <div className="flex gap-3 mt-5">
            <button onClick={save} disabled={saving || !form.name.trim()} className="btn-red px-6 py-2.5 text-sm">
              {saving ? 'Saving...' : editId ? 'Save Changes' : 'Add Athlete'}
            </button>
            <button onClick={() => { setShowForm(false); setForm(emptyForm()); setEditId(null) }}
              className="btn-grey px-6 py-2.5 text-sm">Cancel</button>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div className="card p-10 text-center">
          ...
          <p className="text-gray-400">{search ? 'No athletes match your search.' : 'No athletes yet. Add your first one!'}</p>
        </div>
      ) : (
                ...
                <div className="px-5 pb-5 border-t border-gray-700 pt-4">
                  ...
                  {confirmDelete === athlete.id ? (
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <button onClick={() => remove(athlete.id)} className="btn-red px-4 py-1.5 text-sm">Yes, Remove</button>
                      <button onClick={() => setConfirmDelete(null)} className="btn-grey px-4 py-1.5 text-sm">Cancel</button>
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      <button onClick={() => startEdit(athlete)} className="btn-grey px-4 py-2 text-sm">✏️ Edit</button>
