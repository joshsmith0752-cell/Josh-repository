...
type Competition = {
  ...
  division: string; score: number | null; placement: string; judges_notes: string
}

const empty = (): Omit<Competition, 'id'> => ({
  name: '', date: '', location: '', division: '', score: null, placement: '', judges_notes: '',
})
...
export default function CompetitionsPage() {
  ...
  async function save() {
    ...
    if (!user) return
    if (editId) await supabase.from('competitions').update(form).eq('id', editId)
    else await supabase.from('competitions').insert({ ...form, user_id: user.id })
    setShowForm(false); setForm(empty()); setEditId(null); setSaving(false); load()
  }

  async function remove(id: string) {
    if (!confirm('Delete this competition?')) return
    await supabase.from('competitions').delete().eq('id', id); load()
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <button onClick={() => { setShowForm(true); setForm(empty()); setEditId(null) }} className="btn-red px-4 py-2 text-sm">
          + Add Competition
        </button>
      </div>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input type={f.type} placeholder={f.placeholder}
                  ...
                  onChange={e => setForm(prev => ({ ...prev, [f.key]: f.type === 'number' ? parseFloat(e.target.value) || null : e.target.value }))}
                  className="input" />
              </div>
            ))}
          </div>
          <div className="mt-4">
            <label className="block text-xs text-gray-400 mb-1">Judge's Notes</label>
            <textarea rows={3} placeholder="Notes from judges..." value={form.judges_notes}
              onChange={e => setForm(prev => ({ ...prev, judges_notes: e.target.value }))}
              className="input resize-none" />
          </div>
          <div className="flex gap-3 mt-4">
            <button onClick={save} disabled={saving || !form.name} className="btn-red px-5 py-2 text-sm">
              {saving ? 'Saving...' : 'Save'}
            </button>
            <button onClick={() => setShowForm(false)} className="btn-grey px-5 py-2 text-sm">Cancel</button>
          </div>
        </div>
      )}
          ...
          {competitions.map(c => (
            <div key={c.id} className="card overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-gray-700/20 transition"
                ...
                <div className="flex items-center gap-4">
                  {c.placement && <span className={`font-extrabold text-lg ${placementColor(c.placement)}`}>{c.placement}</span>}
                  {c.score != null && <span className="text-gray-300 text-sm font-medium">{c.score} pts</span>}
              ...
              {expanded === c.id && (
                <div className="px-5 pb-5 border-t border-gray-700 pt-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => startEdit(c)} className="btn-grey px-4 py-2 text-sm">Edit</button>
                    <button onClick={() => remove(c.id)}
                      className="bg-red-900/30 hover:bg-red-900/60 text-red-400 font-semibold px-4 py-2 rounded-xl text-sm transition">Delete</button>
                  </div>
                </div>
              )}
            </div>
